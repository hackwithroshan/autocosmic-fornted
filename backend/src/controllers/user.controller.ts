
import { Request, Response, NextFunction } from 'express';
// FIX: Added 'express' to make the global Express namespace available for Multer types.
import 'express';
import prisma from '../prisma';
import bcrypt from 'bcryptjs';
import { AuthRequest } from '../middlewares/auth.middleware';

// FIX: Add explicit types for req, res, next
export const getDashboardData = async (req: Request, res: Response, next: NextFunction) => {
    const userId = (req as AuthRequest).user!.id;
    try {
        const [profile, orders, addresses, supportTickets, paymentMethods, chatSession] = await Promise.all([
            prisma.user.findUnique({ where: { id: userId }, select: { id: true, name: true, email: true, phone: true, dateOfBirth: true, profilePictureUrl: true, joinDate: true, wishlistProductIds: true, recentlyViewedProductIds: true, role: true } }),
            prisma.order.findMany({ where: { userId }, orderBy: { orderDate: 'desc' }, include: { items: { include: { product: true, variant: true } } } }),
            prisma.address.findMany({ where: { userId } }),
            prisma.supportTicket.findMany({ where: { userId }, orderBy: { lastUpdated: 'desc' } }),
            prisma.paymentMethod.findMany({ where: { userId } }),
            prisma.chatSession.findFirst({ where: { userId }})
        ]);
        res.json({ profile, orders, addresses, supportTickets, paymentMethods, chatSession });
    } catch (error) {
        next(error);
    }
};

// FIX: Add explicit types for req, res, next
export const updateProfile = async (req: Request, res: Response, next: NextFunction) => {
    const userId = (req as AuthRequest).user!.id;
    const { name, phone, dateOfBirth, profilePictureUrl } = req.body;
    try {
        const user = await prisma.user.update({
            where: { id: userId },
            data: { name, phone, dateOfBirth, profilePictureUrl },
        });
        const { password, ...userResponse } = user;
        res.json(userResponse);
    } catch (error) {
        next(error);
    }
};

// FIX: Add explicit types for req, res, next
export const changePassword = async (req: Request, res: Response, next: NextFunction) => {
    const userId = (req as AuthRequest).user!.id;
    const { current, new: newPassword } = req.body;
    try {
        const user = await prisma.user.findUnique({ where: { id: userId } });
        if (!user) return res.status(404).json({ message: "User not found" });

        const isPasswordValid = await bcrypt.compare(current, user.password);
        if (!isPasswordValid) return res.status(401).json({ message: "Invalid current password" });

        const hashedNewPassword = await bcrypt.hash(newPassword, 10);
        await prisma.user.update({
            where: { id: userId },
            data: { password: hashedNewPassword }
        });

        res.json({ success: true, message: "Password updated successfully" });
    } catch (error) {
        next(error);
    }
};

// FIX: Add explicit types for req, res, next
export const getAddresses = async (req: Request, res: Response, next: NextFunction) => {
    const userId = (req as AuthRequest).user!.id;
    try {
        const addresses = await prisma.address.findMany({ where: { userId } });
        res.json(addresses);
    } catch (error) { next(error); }
};
// FIX: Add explicit types for req, res, next
export const addAddress = async (req: Request, res: Response, next: NextFunction) => {
    const userId = (req as AuthRequest).user!.id;
    try {
        const address = await prisma.address.create({ data: { ...req.body, userId } });
        res.status(201).json(address);
    } catch (error) { next(error); }
};
// FIX: Add explicit types for req, res, next
export const updateAddress = async (req: Request, res: Response, next: NextFunction) => {
    const { id } = req.params;
    try {
        const address = await prisma.address.update({ where: { id }, data: req.body });
        res.json(address);
    } catch (error) { next(error); }
};
// FIX: Add explicit types for req, res, next
export const deleteAddress = async (req: Request, res: Response, next: NextFunction) => {
    const { id } = req.params;
    try {
        await prisma.address.delete({ where: { id } });
        res.status(204).send();
    } catch (error) { next(error); }
};

// FIX: Add explicit types for req, res, next
export const getUserOrders = async (req: Request, res: Response, next: NextFunction) => {
    const userId = (req as AuthRequest).user!.id;
    try {
        const orders = await prisma.order.findMany({
            where: { userId },
            orderBy: { orderDate: 'desc' },
            include: { items: { include: { product: true, variant: true } } }
        });
        res.json(orders);
    } catch (error) { next(error); }
};

// FIX: Add explicit types for req, res, next
export const validateCoupon = async (req: Request, res: Response, next: NextFunction) => {
    const { code } = req.body;
    try {
        const coupon = await prisma.coupon.findUnique({ where: { code } });
        if (!coupon || !coupon.isActive) {
            return res.status(404).json({ message: 'Coupon not found or is inactive.' });
        }
        res.json({ success: true, coupon });
    } catch (error) { next(error); }
};

// FIX: Add explicit types for req, res, next
export const toggleWishlist = async (req: Request, res: Response, next: NextFunction) => {
    const userId = (req as AuthRequest).user!.id;
    const { productId } = req.body;
    try {
        const user = await prisma.user.findUnique({ where: { id: userId }});
        if (!user) return res.status(404).json({ message: "User not found" });

        const wishlist = user.wishlistProductIds;
        const newWishlist = wishlist.includes(productId)
            ? wishlist.filter(id => id !== productId)
            : [...wishlist, productId];
        
        await prisma.user.update({
            where: { id: userId },
            data: { wishlistProductIds: newWishlist }
        });
        res.json({ success: true, wishlist: newWishlist });
    } catch (error) {
        next(error);
    }
};

// FIX: Add explicit types for req, res, next
export const addRecentlyViewed = async (req: Request, res: Response, next: NextFunction) => {
    const userId = (req as AuthRequest).user!.id;
    const { productId } = req.body;
    try {
        const user = await prisma.user.findUnique({ where: { id: userId }});
        if (!user) return res.status(404).json({ message: "User not found" });
        
        const recent = user.recentlyViewedProductIds;
        const newRecent = [productId, ...recent.filter(id => id !== productId)].slice(0, 10);

        await prisma.user.update({
            where: { id: userId },
            data: { recentlyViewedProductIds: newRecent }
        });
        res.json({ success: true, recentlyViewed: newRecent });
    } catch (error) {
        next(error);
    }
};

// FIX: Add explicit types for req, res, next
export const getSupportTickets = async (req: Request, res: Response, next: NextFunction) => {
    const userId = (req as AuthRequest).user!.id;
    try {
        const tickets = await prisma.supportTicket.findMany({ where: { userId }, orderBy: { lastUpdated: 'desc' } });
        res.json(tickets);
    } catch (error) { next(error); }
};
// FIX: Add explicit types for req, res, next
export const createSupportTicket = async (req: Request, res: Response, next: NextFunction) => {
    const userId = (req as AuthRequest).user!.id;
    try {
        const ticket = await prisma.supportTicket.create({ data: { ...req.body, userId, seenByAdmin: false } });
        res.status(201).json(ticket);
    } catch (error) { next(error); }
};
// FIX: Add explicit types for req, res, next
export const addMessageToTicket = async (req: Request, res: Response, next: NextFunction) => {
    const { id } = req.params;
    const { messages } = req.body; // Expecting the full message array
    try {
        const originalTicket = await prisma.supportTicket.findUnique({ where: { id } });
        if (!originalTicket) {
            return res.status(404).json({ message: 'Ticket not found' });
        }

        let newStatus = originalTicket.status;
        if (newStatus === 'Resolved' || newStatus === 'Closed') {
            newStatus = 'In Progress';
        }

        const ticket = await prisma.supportTicket.update({
            where: { id },
            data: {
                messages,
                status: newStatus,
                seenByAdmin: false, // Mark as unread for admin
                lastUpdated: new Date().toISOString()
            }
        });
        res.json(ticket);
    } catch (error) {
        next(error);
    }
};

export const getTicketById = async (req: Request, res: Response, next: NextFunction) => {
    const { id } = req.params;
    const userId = (req as AuthRequest).user!.id;
    try {
        const ticket = await prisma.supportTicket.findFirst({
            where: { id, userId }
        });
        if (!ticket) {
            return res.status(404).json({ message: 'Ticket not found or access denied' });
        }
        res.json(ticket);
    } catch (error) { next(error); }
}


// --- NEW LIVE CHAT CONTROLLERS ---
export const getUserChatSession = async (req: Request, res: Response, next: NextFunction) => {
    const userId = (req as AuthRequest).user!.id;
    try {
        let session = await prisma.chatSession.findUnique({
            where: { userId },
        });
        if (!session) {
            session = await prisma.chatSession.create({
                data: { userId }
            });
        }
        res.json(session);
    } catch (error) {
        next(error);
    }
};

export const getChatMessagesForUser = async (req: Request, res: Response, next: NextFunction) => {
    const userId = (req as AuthRequest).user!.id;
    try {
        const session = await prisma.chatSession.findUnique({ where: { userId } });
        if (!session) {
            return res.json([]); // No session, no messages
        }
        const messages = await prisma.chatMessage.findMany({
            where: { sessionId: session.id },
            orderBy: { timestamp: 'asc' }
        });
        res.json(messages);
    } catch (error) {
        next(error);
    }
};

export const sendUserMessage = async (req: Request, res: Response, next: NextFunction) => {
    const userId = (req as AuthRequest).user!.id;
    const { text, attachmentUrl, attachmentType } = req.body;
    try {
        let session = await prisma.chatSession.findUnique({ where: { userId } });
        if (!session) {
            session = await prisma.chatSession.create({ data: { userId } });
        }
        
        const message = await prisma.chatMessage.create({
            data: {
                sessionId: session.id,
                sender: 'user',
                text,
                attachmentUrl,
                attachmentType
            }
        });

        // Update session's lastUpdated timestamp
        await prisma.chatSession.update({
            where: { id: session.id },
            data: { lastUpdated: new Date() }
        });

        res.status(201).json(message);
    } catch (error) {
        next(error);
    }
};

export const uploadChatAttachment = async (req: Request, res: Response, next: NextFunction) => {
    // This assumes the `upload` middleware from cloudinary.ts has run and attached the file object
    const file = req.file as Express.Multer.File & { path: string };
    if (!file) {
        return res.status(400).json({ message: 'No file uploaded.' });
    }
    // The `path` property here is the secure_url from Cloudinary
    res.json({ url: file.path });
};